...
class SyncServer(Server):
    ...
    def update_agent(
        ...
        if request.memory_blocks:
            letta_agent.load_memory_blocks(request.memory_blocks)
    ...
    def get_agent_block_by_label(self, user_id: str, agent_id: str, label: str) -> Block:
        """get an agent block by label"""
        # TODO: implement at ORM?
        for block_id in self.blocks_agents_manager.list_block_ids_for_agent(agent_id=agent_id):
            block = self.block_manager.ge(block_id=block_id, actor=self.user_manager.get_user_by_id(user_id=user_id))
            if block.label == label:
                return block
        return None
    def create_or_fetch_block_by_label(self, user_id: str,label: str) -> Block:
        """Get a block by label, if it doesn't exist, create it"""
        block = self.block_manager.create_or_fetch_block_by_label(label=label, actor=self.user_manager.get_user_by_id(user_id=user_id))
        if block.label == label:
            return block
        return None
