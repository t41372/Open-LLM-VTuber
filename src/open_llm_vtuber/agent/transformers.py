...
def sentence_divider(faster_first_response: bool = True, segment_method: str = "pysbd", valid_tags: List[str] = None):
    """
    ...
    def decorator(
        ...
        async def wrapper(*args, **kwargs) -> AsyncIterator[Tuple[SentenceWithTags, Actions]]:
            ...
            async for sentence in sentence_stream:
                ...
                if not any(tag.state in [TagState.START, TagState.END] for tag in sentence.tags):
...
def display_processor():
    ...
    def decorator(
        ...
        @wraps(func)
        async def wrapper(*args, **kwargs) -> AsyncIterator[Tuple[SentenceWithTags, str, Actions]]:
