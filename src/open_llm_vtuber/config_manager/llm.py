...
class OllamaConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "base_url": Description(
            en="Base URL for Ollama API", zh="Ollama API 的基础 URL"
        ),
        "llm_api_key": Description(en="API key for authentication", zh="API 认证密钥"),
        "organization_id": Description(en="Organization ID for the API", zh="组织 ID"),
        "project_id": Description(en="Project ID for the API", zh="项目 ID"),
        "model": Description(
            en="Name of the LLM model to use", zh="要使用的语言模型名称"
        ),
...
class ClaudeConfig(I18nMixin):
    ...
    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "base_url": Description(
            en="Base URL for Claude API", zh="Claude API 的基础 URL"
        ),
        "llm_api_key": Description(en="API key for authentication", zh="API 认证密钥"),
        "model": Description(
...
class FakeLLMConfig(I18nMixin):
    some_value: bool = Field(True, alias="some_value")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "some_value": Description(en="some value", zh="一些东西"),
    }
...
class LLMConfig(I18nMixin):
    ...
    llm_provider: Literal[
        "ollama_llm", "memgpt", "mem0", "claude_llm", "llama_cpp_llm", "fake_llm"
    ...
    fake_llm: Optional[FakeLLMConfig] = Field(default=FakeLLMConfig(), alias="fake_llm")

    DESCRIPTIONS: ClassVar[Dict[str, Description]] = {
        "llm_provider": Description(en="LLM Provider", zh="语言模型提供者"),
        ...
        "claude_llm": Description(
            en="Configuration for Claude API", zh="Claude API 配置"
        ),
        "llama_cpp_llm": Description(
            en="Configuration for Llama CPP that runs directly in the backend",
            zh="LlamaCpp 配置 (直接在后端运行)",
        ),
        "mem0": Description(en="Configuration for Mem0", zh="Mem0 配置"),
        "memgpt": Description(en="Configuration for MemGPT", zh="MemGPT 配置"),
    }
